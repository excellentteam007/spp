...
class PropertyManage:
    ...
    # 增加物业的所有动作集
    def do_add_property(self, data):

        self.click_add_property_button()
        self.input_property_name(data['property_name'])
        self.input_property_phone(data['property_phone'])
        self.input_property_introduce(data['property_intro'])
        self.input_property_credential(data['property_cred'])
        self.input_property_password(data['property_pwd'])
        self.click_add_button()

    # 增加物业用例的断言
    def get_add_property_actual(self, data):
        self.do_add_property(data)
        time.sleep(2)
        actual_e = self.e.get_element('add_property_actual')
        content = self.ui.get_text(actual_e)
        if content == "增加成功":
            actual = "增加成功"
        else:
            actual = "增加失败"
    ...
    # 通过名字进行搜索动作集
    def do_property_search_by_name(self, data):
        # self.click_user_management_button()
        # self.click_info_property_button()
        # self.skip_iframe()
        self.input_search_name(data['property_name'])
        self.click_search_enter()

    # 通过姓名搜索的断言
    def get_property_search_by_name_actual(self, data):
        self.do_property_search_by_name(data)
        time.sleep(3)
        actual_e = self.e.get_element('property_search_by_name_actual')
        time.sleep(1)
        result = self.ui.get_text(actual_e)
        # time.sleep(1)
        if result == data['property_name']:
            actual = "查找成功"
        else:
            actual = "查找失败"
        return actual
    ...
    #根据电话搜索物业方的动作集
    def do_property_search_by_phone(self,data):
        # self.click_user_management_button()
        # self.click_info_property_button()
        # self.skip_iframe()
        ...
        time.sleep(2)
        # self.input_property_search_phone(int(data['property_phone']))
        self.input_property_search_phone(data)
        self.click_search_enter()

    # 物业方信息通过电话搜索的断言
    def get_property_search_by_phone_actual(self, data):
        data1 = int(data['property_phone'])
        # print(data1)
        self.do_property_search_by_phone(data1)
        time.sleep(3)
        actual_e = self.e.get_element('property_search_by_phone_actual')
        time.sleep(2)
        result = self.ui.get_text(actual_e)
        if result == data['property_phone']:
            actual = '查找成功'
        else:
            actual = '查找失败'
        return actual
    ...
    # 修改物业方用户信息的动作集
    def do_property_change_info(self, data):
        # self.click_user_management_button()
        # self.click_info_property_button()
        # self.skip_iframe()
        self.input_search_name(data['property_name'])
    ...
    # 物业方修改用户信息的断言
    def get_property_change_phone_actual(self, data):
        self.do_property_change_info(data)
        time.sleep(5)
...
if __name__ == '__main__':
    ...
    # print(PropertyManage().get_property_change_phone_actual()
